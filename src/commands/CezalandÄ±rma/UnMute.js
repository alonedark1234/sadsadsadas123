...
module.exports = {
  ...
  run: async (client, message, args, embed) => {
    ...
    .setLabel("Chat Mute")
    .setStyle("SECONDARY")
    .setEmoji("1142174891739730030");

    let vmute = new MessageButton()
    .setCustomId("vmute")
    .setLabel("Voice Mute")
    .setStyle("SECONDARY")
    .setEmoji("1142174853181489252");
...
const log = embed
        ...
        .setColor("#2f3136")
        .setDescription(`
${star} ${member.toString()} Adlı Kişinin Chat Mutesi Kaldırıldı
...
${miniicon} Mute Kaldıran Kişi : ${message.author} (\`${message.author.id}\`)`)
    ...
    const sex = embed
        ...
        .setColor("#2f3136")
        .setDescription(`
${star} ${member.toString()} Adlı Kişinin Ses Mutesi Kaldırıldı
      
${miniicon} Sesli Muteyi Kaldıran Kişi : ${message.author} (\`${message.author.id}\`)`)
        .setFooter({ text:`${moment(Date.now()).format("LLL")}` })
      message.guild.channels.cache.get(conf.cmuteLogChannel).wsend({ embeds : [sex]}); 
