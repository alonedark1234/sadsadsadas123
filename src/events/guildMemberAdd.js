...
const otoreg = await otokayit.findOne({ userID: member.id })
 const tagModedata = await regstats.findOne({ guildID: allah.GuildID })
  ...
  const kayitchannel = member.guild.channels.cache.get("1131944633191116851");
...
${member} (\`${member.id}\`) ${member.guild.name} Sunucumuza Hoş Geldin,
...
<@&914479013907562515>
`});
channel.wsend({ content:`<a:dp_sag_ok:1143529154906706021> ${member}, sunucuya katıldı! Davet Eden: **Sunucu Özel URL** :tada: <t:${Math.floor(Date.now() / 1000)}:R> \`katıldı.\``})
return }
...
channel.wsend({ content:`<a:dp_sag_ok:1143529154906706021> ${member}, ${usedInvite.inviter.tag} davetiyle katıldı! (**${total}** ${green}) <t:${Math.floor(Date.now() / 1000)}:R> \`katıldı.\` `})
...
<@&914479013907562515>
`});
channel.wsend({ content:`${member}, ${usedInvite.inviter.tag} davetiyle katıldı! (**${total}** ${green}) <t:${Math.floor(Date.now() / 1000)}:R> \`katıldı.\``})
}
