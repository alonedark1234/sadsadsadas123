    ...
    if (conf.staffs.some(x => user.member.roles.cache.has(x))) {
      ...
      if (coinData && client.ranks.some((x) => x.coin >= coinData.coin)) {
        ...
        if (newRank && Array.isArray(newRank.role) && !newRank.role.some(x => user.member.roles.cache.has(x)) || newRank && !Array.isArray(newRank.role) && !user.member.roles.cache.has(newRank.role)) {
          ...
          client.channels.cache.find(x => x.name == "rank-log").send({ content:`${user.member.toString()} üyesi **${coinData.coin}** coin hedefine ulaştı ve **${Array.isArray(newRank.role) ? newRank.role.map(x => `${user.guild.roles.cache.get(x).name}`).join(", ") : `${user.guild.roles.cache.get(newRank.role).name}`}** rolü verildi! :tada: :tada:`});
        }
      }
    }
  }
