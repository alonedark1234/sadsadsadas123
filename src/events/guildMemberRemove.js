...
module.exports = async (member) => {
  ...
  if (!inviteMemberData) {
    channel.wsend({ content: `\`${member.user.tag}\` sunucumuzdan ayrıldı ama kim tarafından davet edildiğini bulamadım. <t:${Math.floor(Date.now() / 1000)}:R> \`ayrıldı.\``});
  } else {
    ...
    channel.wsend({ content:`\`${member.user.tag}\` sunucumuzdan ayrıldı. ${inviter.tag} tarafından davet edilmişti. (**${total}** davet) <t:${Math.floor(Date.now() / 1000)}:R> \`ayrıldı.\``});
